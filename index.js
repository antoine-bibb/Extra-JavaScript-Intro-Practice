...
function convert(usd){
  const euro= usd * 0.85;
  return ` your USD has been converted to ${euro}`
}
convert(5);
...
function avail (arr, code){
  for(let i = 0; i <arr.length;
    i++)
    if (airports[i].code === code){
      console.log(airports[i].tz);
    }
}
avail(airports, "ABR");
